...
func TestDefaultConfig(t *testing.T) {
	...
	if len(cfg.ProtectionPatterns) != 3 {
		t.Fatalf(
			"expected 3 protection patterns, got %d",
			len(cfg.ProtectionPatterns),
		)
	}
	...
	for i, p := range expected {
		if cfg.ProtectionPatterns[i] != p {
			t.Errorf(
				"protection pattern %d: expected %q, got %q",
				i,
				p,
				cfg.ProtectionPatterns[i],
			)
		}
	}
	...
	if cfg.PrunePresets[0].Name != "Dangling Only" {
		t.Errorf(
			"expected first preset name 'Dangling Only', got %q",
			cfg.PrunePresets[0].Name,
		)
	}

	if cfg.PrunePresets[1].Name != "Build Cache" {
		t.Errorf(
			"expected second preset name 'Build Cache', got %q",
			cfg.PrunePresets[1].Name,
		)
	}
}
...
func TestSaveAndLoad(t *testing.T) {
	...
	if len(loaded.ProtectionPatterns) != 1 ||
		loaded.ProtectionPatterns[0] != "*custom*" {
		t.Errorf("unexpected ProtectionPatterns: %v", loaded.ProtectionPatterns)
	}

	if len(loaded.PrunePresets) != 1 ||
		loaded.PrunePresets[0].Name != "Test Preset" {
		t.Errorf("unexpected PrunePresets: %v", loaded.PrunePresets)
	}

	if loaded.BaselinePath != "/tmp/test-baseline.json" {
		t.Errorf(
			"expected BaselinePath '/tmp/test-baseline.json', got %q",
			loaded.BaselinePath,
		)
	}
}
...
func TestLoadNonExistent(t *testing.T) {
	...
	if cfg.RefreshInterval != def.RefreshInterval {
		t.Errorf(
			"expected default RefreshInterval=%d, got %d",
			def.RefreshInterval,
			cfg.RefreshInterval,
		)
	}
	...
	if len(cfg.ProtectionPatterns) != len(def.ProtectionPatterns) {
		t.Errorf(
			"expected %d protection patterns, got %d",
			len(def.ProtectionPatterns),
			len(cfg.ProtectionPatterns),
		)
	}
}
